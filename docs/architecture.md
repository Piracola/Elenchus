...
> 结构优化优先级、冗余代码清单与超长文件分析请见 [CODE_OPTIMIZATION_AUDIT_2026-04-02.md](./CODE_OPTIMIZATION_AUDIT_2026-04-02.md)。
...
- `backend/app/runtime/bus.py`：运行事件广播与持久化总线；当前 `RuntimeBus` 是运行时事件分发的唯一主入口，`EventStreamGateway` / `ConnectionHub` 兼容名称已收紧为过渡壳层，不再通过聚合导出与依赖别名继续暴露
...
- `backend/prompts/`：标准模式与事实核查相关提示词目录，包含辩手基础提示词、正反方补充提示词、裁判提示词与事实核查提示词
- `backend/prompts/sophistry/`：诡辩实验模式提示词目录，包含辩手基础提示词、正反方补充提示词与观察员提示词
- `backend/app/agents/prompt_loader.py`：标准模式提示词加载入口，负责拼接基础辩手提示词与角色补充提示词，并提供裁判提示词读取
...
- 架构收敛与历史审查背景：读 [archive/2026-03-17-audit-summary.md](./archive/2026-03-17-audit-summary.md)
- 2026-03-18 已核实整改结论与原始背景：分别读 [CODE_QUALITY_REPORT_2026-03-18.md](./CODE_QUALITY_REPORT_2026-03-18.md) 与 [archive/2026-03-18-code-audit-background.md](./archive/2026-03-18-code-audit-background.md)
