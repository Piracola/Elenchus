...
# Backward compatibility: module-level singleton (deprecated)
# Use app.dependencies.get_search_factory() instead
# This will be removed in a future version
_factory_instance: SearchProviderFactory | None = None


def _get_factory_instance() -> SearchProviderFactory:
    """Get or create the singleton factory instance for backward compatibility."""
    global _factory_instance
    if _factory_instance is None:
        _factory_instance = SearchProviderFactory()
    return _factory_instance


# Class method aliases for backward compatibility
# These delegate to the singleton instance and will be removed in a future version
def _warn_deprecated():
    logger.warning(
        "Calling SearchProviderFactory class methods is deprecated. "
        "Use app.dependencies.get_search_factory() instead."
    )


# Expose class-method-like interface for backward compatibility
SearchProviderFactory.get_current_provider = classmethod(  # type: ignore
    lambda cls: _get_factory_instance().get_current_provider()
)
SearchProviderFactory.set_provider = classmethod(  # type: ignore
    lambda cls, provider: _get_factory_instance().set_provider(provider)
)
SearchProviderFactory.get_available_providers = classmethod(  # type: ignore
    lambda cls: _get_factory_instance().get_available_providers()
)
SearchProviderFactory.get_provider = classmethod(  # type: ignore
    lambda cls: _get_factory_instance().get_provider()
)
SearchProviderFactory.search = classmethod(  # type: ignore
    lambda cls, query, num_results=5: _get_factory_instance().search(query, num_results)
)
SearchProviderFactory.close = classmethod(  # type: ignore
    lambda cls: _get_factory_instance().close()
)
