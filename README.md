...
│   │   │   ├── intervention_manager.py  # 用户介入管理
│   │   │   └── log_service.py       # 日志配置服务
│   │   ├── constants.py        # 共享常量
│   │   ├── 📁 search/          # 搜索引擎接口
│   │   │   ├── factory.py         # 搜索工厂（单例）
│   │   │   ├── base.py            # 搜索基类
│   │   │   ├── duckduckgo.py      # DuckDuckGo 搜索（默认）
...
│   │   ├── 📁 stores/          # Zustand 状态仓库
│   │   ├── 📁 hooks/           # 自定义 Hooks
│   │   │   ├── useDebateWebSocket.ts # WebSocket 管理
│   │   │   ├── useAgentConfigs.ts    # Agent 配置管理
│   │   │   └── ...
│   │   ├── 📁 api/             # API 客户端
│   │   ├── 📁 utils/           # 工具函数
│   │   ├── 📁 types/           # TypeScript 类型
...
```javascript
const ws = new WebSocket('ws://localhost:8001/api/ws/{session_id}');
